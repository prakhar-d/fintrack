...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    const res = await axios.post('https://fintrack-z9l.onrender.com/api/auth/login', { email, password });
  ...
  const register = async (name, email, password) => {
    const res = await axios.post('https://fintrack-z9l.onrender.com/api/auth/register', { name, email, password });
    setToken(res.data.token);
    setUser(res.data.user);
    localStorage.setItem('fintrack_token', res.data.token);
    localStorage.setItem('fintrack_user', JSON.stringify(res.data.user));
